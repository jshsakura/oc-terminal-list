...
- **열기 (Open) deep-link — a plain-text URL in the body, NOT an inline button.** Each notification can carry a `🔗 <base>/?open=<sessionId>` line that opens the web app straight to the pane that fired it. The frontend (`hooks/useDeepLinkOpen.js`) reads `?open=`, finds the tab/pane whose `sessionId` (local) or `id` (remote) matches, activates it, then strips the param. It waits for login+restore before giving up on a not-yet-loaded session; a session that's genuinely gone just clears the param quietly.
  - **Why not an inline URL button:** a Telegram inline URL button opens in Telegram's in-app WebView, which has none of the app's login cookies, and the Bot API offers no flag to force the external browser. A plain-text link can be long-pressed → "Open in browser" / "Copy Link", so it lands in the real browser. It sits **right under the label** (`build_done_message`, `open_url=`) so a long excerpt truncating the body can't drop it.
  - The base URL **must be configured** — a background worker has no request to derive it from. `PUBLIC_BASE_URL` env wins; otherwise the settings-UI value (`public_base_url` config key). **No base URL → no link** (a broken link is worse than none), so the notification still sends.
