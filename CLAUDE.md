...
복구를 위한 장치인데(CLAUDE.md 의 원격 세션 소멸 절), 사용자가 **직접 지운** 경우엔
정반대로 작동한다 — 지워도 곧바로 되살아나고 화면에서는 "지워도 새로고침하면 다시 뜬다"
로 보인다. 우리 쪽에서 죽인 것과 저절로 사라진 것은 겉으로 구별되지 않는다.

→ `session_tombstones`. 죽인 쪽이 표를 남기고, 브리지는 그 표가 있으면 만들지 않는다.

- **수명이 짧다(90s).** 재접속 한 번을 막는 장치지 영구 차단이 아니다 — 나중에 같은
  이름으로 새로 여는 것은 정상이고 막으면 안 된다.
- **거절은 `create` 를 끄는 것이지 연결을 막는 게 아니다.** 살아 있는 다른 세션에 붙는
  것까지 막으면 그 호스트가 통째로 안 열린다.
- `force`(kill-server)는 표를 남기지 않는다. 그건 "이 호스트를 통째로 리셋" 이라 다음
  연결이 새 세션을 여는 게 맞다.
