...
⚠️ **아직 남은 것: `terminal_send`/`terminal_key` 의 `@working`.** 같은 뿌리로 원격을
빼먹지만, 고치려면 배달 전에 호스트당 SSH 왕복이 **한 겹 더** 붙는다. 이 경로의 시간
예산은 이미 빠듯하다(백엔드 `HOST_DEADLINE=20s` < 호출자 30s, 위 함정 참고) — 두 단계를
직렬로 태우면 호출자 타임아웃을 넘겨 **배달됐는데 실패로 읽혀 재시도가 중복 전송**이 된다.
비용/정확도 트레이드오프라 의도적으로 남겼다.
