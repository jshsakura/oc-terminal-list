...
- **열기 (Open) deep-link button.** Each notification can carry a `🔗 열기` **URL button** (not a callback) that opens the web app straight to the pane that fired it: `<base>/?open=<sessionId>`. The frontend (`hooks/useDeepLinkOpen.js`) reads `?open=`, finds the tab/pane whose `sessionId` (local) or `id` (remote) matches, activates it, then strips the param. It waits for login+restore before giving up on a not-yet-loaded session; a session that's genuinely gone just clears the param quietly.
  ...
  - URL buttons and callback buttons coexist: `telegram_client._build_keyboard` puts each URL button on its own full-width row above the callback row (계속/중단).
