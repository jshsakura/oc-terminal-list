...
크기를 정한다.** 폰이 48컬럼으로 붙으면 같은 세션을 보고 있던 PC 화면이 그 폭으로 짤리고,
**한 번 좁게 리랩된 스크롤백은 폰이 떨어져도 되돌아오지 않는다.** 이건 성능 문제가 아니라
사실상 데이터 손상이다.

그런데 부팅은 **모든 탭·모든 pane** 에 붙었다 — 폰에서 앱을 열면 열지도 않은 탭의 세션까지
전부 폰 폭으로 리랩됐다. 그리고 그 소켓들은 어차피 60초 뒤 `INACTIVE_PANE_GRACE_MS` 로
닫힌다. **버릴 연결을 위해 남의 화면을 부수고 있었다.**
