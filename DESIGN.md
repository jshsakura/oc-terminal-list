...
## 17. 좁은 정보 패널의 프로세스 카드

- 프로세스 이름·PID·명령은 카드 첫 줄 전체 폭을 쓰고, 배지·CPU/RSS·종료 액션은 둘째 줄에 둔다.
- 종료 액션은 `ProcessActionButton` 하나로 warning/danger 톤만 바꾸며, 아이콘은 12px여도 히트 영역은 `space['6']`(24px)를 유지한다.
- 긴 이름과 명령은 카드 폭을 늘리지 않고 말줄임표로 접는다. 패널 최소 폭 180px에서도 두 종료 액션과 통계가 겹치면 안 된다.
