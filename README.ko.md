...
Apple의 한글 IME가 형편없어서입니다.

Terminal.app이나 iTerm2에서 한글을 입력하면 IME 조합 레이어가 터미널 입력 처리와 충돌합니다. 자음이 빠지거나, 백스페이스가 엉뚱한 글자를 지우거나, 단어 중간에 커서가 튀는 현상이 생깁니다. 근본 원인은 구조적입니다 — 네이티브 터미널은 바이트 스트림 기반으로 설계됐고, 한글 입력은 OS IME가 잘못된 레이어에서 관리하려는 상태형 조합이 필요합니다.

브라우저 기반 터미널은 이 문제를 완전히 우회합니다. 브라우저 자체 텍스트 입력 파이프라인이 한글 조합을 올바르게 처리하고, xterm.js는 이미 완성된 문자만 받습니다. 이 프로젝트는 그 불편함 하나를 해결하려는 개인용 픽스로 시작해 풀 기능 터미널 워크스페이스로 발전했습니다.
...
한글(CJK 포함)이 올바르게 입력되는, 노트북 스타일 원격 셸보다 빠르고 직관적인 가벼운 터미널 대시보드가 필요할 때 유용합니다.
...
| 파일 | 워크스페이스 범위 파일 탐색기, Monaco 에디터, 생성/이동/삭제 |
...
| 입력 | 빠른입력 패널 + 음성 입력(Web Speech API), 터미널별 명령 이력 (무한 스크롤) |
| UI | 테마, 언어 전환, 모바일 툴바 (롱프레스 컨텍스트 메뉴, 탭 포커스), 반응형 레이아웃 |
| IME | 브라우저 네이티브 조합 — 한글/CJK 입력이 올바르게 동작 |
| 성능 | Gzip, 장기 정적 자산 캐시, 지연 로딩 프론트엔드 청크, Monaco idle prefetch, WebSocket 배치 |
...
- 모바일 툴바 또는 단축키로 빠른입력 패널을 엽니다.
- 명령어를 직접 입력하거나 마이크 버튼으로 Web Speech API를 통해 음성으로 입력합니다.
- **명령 이력** 패널(눈 아이콘)에서 터미널별 명령 이력을 무한 스크롤로 확인합니다.
- 이력 항목을 클릭하면 커서 위치에 삽입됩니다.
