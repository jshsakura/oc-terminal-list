...
🔐 **설치는 사람이 누를 때만 일어난다.** 자동 설치도 자동 시작도 없다.

⚠️ 한때 "안 깔아도 백엔드가 SSH 로 probe 를 띄우는 경로가 그대로 있다" 고 적혀 있었다.
그 폴백은 없앴다(꺼진 기계에 15초를 태워 홈 화면을 멈춰 세웠다). 지금은 **안 깔면
그 호스트의 pane 은 상태가 `statusUnknown` 이고 명령을 받지 못한다.** 여전히 강요하지
않지만, 화면은 그 사실대로 적어야 한다.
