...
# "열기" 링크 버튼 라벨. 콜백이 아니라 URL 버튼이라 눌러도 봇으로 되돌아오지 않고
# 브라우저만 연다.
...
async def notify_agent_done(session_id: str, command: str, title: str,
    ...
    buttons = []
    # "열기" 링크 버튼 — 기준 주소가 설정돼 있을 때만. 계속·중단과 같은 행에 놓여
    # 전폭이 아니라 작게 나온다. 본문에 URL 을 넣으면 알림 미리보기(첫 줄)가 링크로
    # 뭉개지므로 버튼으로 낸다. 주소가 없으면 조용히 생략(깨진 링크보다 낫다).
