...
async def test_notify_adds_open_button_when_base_url_set():
    ...
    본문에 URL 을 넣으면 알림 미리보기(첫 줄)가 링크로 뭉개지므로 버튼으로 낸다.
    """
    ...
    assert "?open=" not in body                    # 링크는 본문이 아니라 버튼에만
...
@pytest.mark.anyio
async def test_open_button_shares_the_row_with_actions():
    """링크 버튼이 계속·중단과 같은 행 — 전폭이 되지 않게(너무 큼 방지)."""
    ...
    assert len(keyboard) == 1                       # 한 행
    assert keyboard[0][0] == {"text": "🔗 열기", "url": "https://x/?open=s1"}
    assert [b["text"] for b in keyboard[0]] == ["🔗 열기", "계속", "중단"]
