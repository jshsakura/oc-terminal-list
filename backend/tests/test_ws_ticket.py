...
import _deps
import main
import tickets  # 티켓 저장소·발급/소비는 main 에서 분리됨
...
def test_ws_ticket_is_single_use_and_path_bound():
    ticket, _expires_at = tickets._create_ws_ticket("admin", "/ws/session-1")

    assert tickets._consume_ws_ticket(ticket, "/ws/session-2") is None
    assert tickets._consume_ws_ticket(ticket, "/ws/session-1") is None

    ticket, _expires_at = tickets._create_ws_ticket("admin", "/ws/session-1")
    assert tickets._consume_ws_ticket(ticket, "/ws/session-1") == "admin"
    assert tickets._consume_ws_ticket(ticket, "/ws/session-1") is None
...
def test_ws_ticket_rejects_non_ws_paths():
    with pytest.raises(HTTPException):
        tickets._create_ws_ticket("admin", "/api/sessions")
...
def test_file_ticket_is_single_use(monkeypatch, tmp_path):
    ...
    ticket, _expires_at = tickets._create_file_ticket("admin", "preview.png")

    assert tickets._consume_file_ticket(ticket) == str(target)
    assert tickets._consume_file_ticket(ticket) is None
...
def test_validate_path_preserves_literal_dotdot_in_filename(monkeypatch, tmp_path):
    ...
    assert _deps.validate_path("a..b.txt") == tmp_path.resolve() / "a..b.txt"
...
def test_validate_path_rejects_parent_traversal(monkeypatch, tmp_path):
    ...
    with pytest.raises(HTTPException):
        _deps.validate_path("../outside.txt")
...
def test_validate_path_rejects_symlink_escape(monkeypatch, tmp_path):
    ...
    with pytest.raises(HTTPException):
        _deps.validate_path("link/secret.txt")
...
def test_validate_path_can_reject_workspace_root(monkeypatch, tmp_path):
    ...
    with pytest.raises(HTTPException):
        _deps.validate_path("/", allow_root=False)
...
def test_verify_endpoint_promotes_legacy_bearer_to_cookie():
    ...
    assert f"{_deps.AUTH_COOKIE_NAME}=legacy-token" in set_cookie
...
def test_logout_clears_auth_cookie():
    ...
    assert res.status_code == 200
    set_cookie = res.headers["set-cookie"]
    assert f"{_deps.AUTH_COOKIE_NAME}=" in set_cookie
