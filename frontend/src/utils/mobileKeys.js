...
const PREFIX = '\x02';   // ctrl+b — tmux 의 기본 프리픽스

/* tmux 프리픽스 키.
 *
 * ⚠️ **프리픽스는 하나다(`\x02`).** 한때 `\x02\x02` 였고, 그 근거는 "이 앱의 pane 은
 * 언제나 tmux 클라이언트 안" 이었다. 팬이 붙는 tmux 바깥에 또 하나가 깔려 있지 않다 —
 * 이중으로 보내면 안쪽이 두 번째를 명령 키로 읽어 아무 일도 안 일어난다.
 *
 * 다른 멀티플렉서의 키는 싣지 않는다. 같은 `C-b` 라도 뒤 글자가 달라 섞어 두면 눌러도
 * 아무 일이 없는 키가 바에 남는다 — 그 실패는 조용하다. 필요하면 커스텀 키로 넣는다
 * (임의 바이트열을 보낼 수 있다). */
